...
# RISC-V whisper.cpp CPU-only build script for reCamera (simple version)
# This script cross-compiles whisper.cpp for RISC-V with basic CPU support only
...
echo "Building whisper.cpp for RISC-V (CPU-only, basic optimizations)..."
...
BUILD_DIR="build-riscv64-simple"
if [ ! -d "$BUILD_DIR" ]; then
    mkdir -p "$BUILD_DIR"
fi
...
echo "Configuring whisper.cpp for RISC-V (basic CPU mode)..."

# Configure with cmake for basic RISC-V build (no vector extensions)
cmake .. \
    ...
    -DCMAKE_C_FLAGS="-O2 -DNDEBUG -march=rv64gc" \
    -DCMAKE_CXX_FLAGS="-O2 -DNDEBUG -march=rv64gc" \
    -DGGML_STATIC=ON \
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        # Show built binaries
        echo "Built binaries:"
        ls -la bin/ 2>/dev/null || echo "Binary directory not found"
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            if [ -f "bin/main" ]; then
                ...
                ssh_exec "chmod +x ~/whisper/whisper-main"
            fi
            ...
            if [ -f "bin/server" ]; then
                ...
                ssh_exec "chmod +x ~/whisper/whisper-server"
            fi
            ...
            if [ -f "bin/stream" ]; then
                ...
                ssh_exec "chmod +x ~/whisper/whisper-stream"
            fi
            ...
            echo "   cd ~/whisper"
            echo "   # Record audio with arecord or use existing audio file"
